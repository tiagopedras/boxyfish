/* Turn a photo of a drawing into an aircraft cut-out.
 *
 * Lifted from the aquarium's fishify.js, which does the same job for fish --
 * an outline on paper is an outline on paper, whatever it is a drawing of:
 *   1. flatten the lighting, so a shadow across the paper doesn't read as ink
 *   2. find the ink, close small gaps so a detached wingtip joins its wing
 ...
 *
 * Which way the aircraft points is not worked out here: the drawing is assumed
 * to be held up nose-right, and the preview lets you turn it if it wasn't.
 */
window.planify = (function(){
...
function planify(source){
  ...
  // close gaps, then anything the border can't reach is inside the aircraft
